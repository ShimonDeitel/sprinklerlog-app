...
struct ContentView: View {
    ...
    @State private var editingEntry: ZoneTaskEntry?
...
struct EntryFormView: View {
    ...
    @State private var stage: String = ZoneTaskEntry.stageOptions.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    Picker("Stage", selection: $stage) {
                        ForEach(ZoneTaskEntry.stageOptions, id: \.self) { Text($0).tag($0) }
                    }
            ...
            .navigationTitle(mode.id == "add" ? "New Zone Task" : "Edit Zone Task")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = ZoneTaskEntry(title: title, stage: stage, date: date, note: note, photoData: photoData)
            store.add(entry)
        case .edit(var entry):
            entry.title = title
            entry.stage = stage
